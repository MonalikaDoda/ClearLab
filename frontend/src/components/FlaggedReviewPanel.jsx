...
function FlaggedReviewPanel({ invoices, onUpdated }) {
	...
	const handleReminder = async (invoice) => {
		...
		try {
			...
			setProviders((current) => ({ ...current, [invoice._id]: result.generatedBy }));
			await onUpdated();
		} catch (requestError) {
